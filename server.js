...
// AUTHENTICATION ROUTES WRITTEN BY COLIN FOR JAIL SESSIONS HOMEWORK //
...
var lobby = function(req, res, next) {
    User.findOne({_id: req.session.uid}, function(err, user){
...
var visitorLounge = function(req, res, next) {
    User.findOne({_id: req.session.uid}, function(err, user){
        if (user.role === 'warden' || user.role === 'guard' || user.role === 'visitor') {
            next();
        } else {
...
var cafeteria = function (req, res, next) {
    User.findOne({_id: req.session.uid}, function(err, user){
...
var cellE = function (req, res, next) {
    User.findOne({_id: req.session.uid}, function(err, user){
...
var cellM = function (req, res, next) {
    User.findOne({_id: req.session.uid}, function(err, user){
...
app.get('/visitors-lounge', visitorLounge, function(req, res, next){
